...
def play_recursive_combat(
    ...
    is_a_winner = False
    won_by_short_circuit = False
    while not is_a_winner:

        if (tuple(deck_1), tuple(deck_2)) in memory:
            is_a_winner = True
            won_by_short_circuit = True
            ...
            if len(deck_1) >= n_1 and len(deck_2) >= n_2:
                sub_won_by_short_circuit, _, sub_deck_2 = play_recursive_combat(
                    copy(deck_1[:n_1]), copy(deck_2[:n_2]), memory=set()
                )

                if sub_won_by_short_circuit or not sub_deck_2:
                    deck_1.extend([n_1, n_2])
                else:
                    deck_2.extend([n_2, n_1])
    ...
    return won_by_short_circuit, deck_1, deck_2
